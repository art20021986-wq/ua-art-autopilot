...
DATE: 2026-09-04
AUTOSTART_CONFIRMED_BY_OWNER: YES
AUTOSTART_SCOPE: TASK112_SANDBOX_BRANCH_ONLY
GLOBAL_AUTOMATIC_MODE: UNCHANGED_MANUAL
DUPLICATE_AUTOSTART_GUARD: active_run_33878083108
SOURCE_TASK: task_103
TARGET: построить и проверить полный изолированный Sandbox/Preview-пакет без Production write
MAX_ROUNDS: 10

# TASK 112 — UA ART: грузинский язык, 8 стран «Авто под заказ» и защита 16 карточек
...
## OWNER AUTOSTART COMMAND — 2026-09-04

Включить автоматический запуск этой задачи немедленно. Выполнять автономно в отдельной Sandbox-ветке, с точной маршрутизацией только на tasks/task_112.md. Production, CRM production data, DNS, VIN, цены, карточки и медиа не изменять. TASK107-R2 3/3 receipt должен быть проверен до запуска. Если полного Sandbox PASS нельзя доказать, продолжить безопасными этапами и указать точный блокер; не выдавать подготовительный пакет за завершённую реализацию.
