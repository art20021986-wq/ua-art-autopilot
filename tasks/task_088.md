...
## Авторизованный повтор после устранения первопричины

Повтор разрешён после установки канонического блока «Комплексная диагностика»
в полную карточку. Запуск выполняется из точного commit SHA и обязан снова
пройти shadow, backup, транзакцию, restart, immediate и delayed cross-control.
