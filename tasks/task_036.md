# TASK 036 — CRM-SPEED-001 correct the single invalid TASK 035 test fixture

## Authority and safety

Correct only the invalid temporary SQLite setup in test_task_035_integration.py. Work only under cloud/. Do not execute Gate A, access PythonAnywhere/network/production, or modify Production, CRM, crm.db, site, bot, media, WSGI, processes, UA-0009, or tasks/.
...
On commit 4a86fb4d3fdd88b56a38af8edbc5feacd3a29b9a:

- py_compile PASS;
- 188 tests executed;
- 187 PASS;
- the only ERROR is inside test_sqlite_ownership_evidence_delegates_to_canonical_function;
- _make_temp_config() already creates table t with column id INTEGER;
- the test then incorrectly runs ALTER TABLE t ADD COLUMN id TEXT, causing sqlite3.OperationalError: duplicate column name: id before the implementation under test is reached.

## Required correction

In test_sqlite_ownership_evidence_delegates_to_canonical_function:

- remove the duplicate ALTER TABLE statement;
- use the already-existing id column;
- insert one deterministic identifier row compatible with cfg["ua0009_id_value"] = "1";
- preserve the monkeypatch/delegation assertion that collect_ua0009_ownership_evidence is called exactly twice, before and after the workload;
- close/commit the temporary connection correctly;
- do not weaken, skip, rename or delete this or any test;
- do not change production implementation modules;
- preserve all other test content byte-for-byte where practical.

The corrected test must actually reach the canonical delegation code and prove len(calls) == 2.
...
Expected: all 188 tests plus any added test pass with zero FAIL/ERROR.
...
- `cloud/crm_speed_optimization/TASK_036_REPORT.md`
- `cloud/latest_status.md`
- `cloud/owner_reply.md`

Do not return or modify implementation files. Status after success is READY_FOR_CONTROLLER_REVIEW_PHASE_E_TEST_FIX, not READY_FOR_GATE_A.
