...
## Доказанная причина — уточнение после первой безопасной попытки

Свежий Gate A TASK 075 подтвердил:

1. Перед первым restart UA-0011 имела канонический `status=sea_loaded`, 35 фото и этап 2.
...
5. После restart аудит CRM зафиксировал точный откат `sea_loaded → kr_bought` в 07:36:51 от actor владельца, при этом `sea_container=ONEYSELGF1046602` сохранился. Это доказало второй дефект: старая/ошибочная кнопка CRM могла понизить этап вопреки уже подтверждённым логистическим данным.

Следовательно, исправляются обе причины: карточка пересобирается после media sync, а нижний слой CRM блокирует понижение ниже доказанного этапа (`sea_container`/`sea_date_out` → минимум этап 2; грузинские данные → минимум 3; украинские → минимум 4).
...
- один marker-bounded wrapper `db.update_card_field`, запрещающий противоречивый регресс этапа;
- одна точечная транзакция `UA-0011.status: kr_bought → sea_loaded` и её audit receipt;
- restart только активного `/home/Carix/start_safe.py` после успешной установки.

Запрещено менять любые иные CRM rows/поля, VIN, цену, описание, фото/видео, индивидуальные страницы и посторонние файлы.

## Обязательная логика

- источник публичного этапа — только CRM `status`; доказанные логистические поля задают нижнюю границу и не дают ошибочно откатить `status` назад;
...
- у CRM до/после разрешено только точечное изменение `UA-0011.status`, её `updated_at` и audit-запись; все прочие строки/поля идентичны;
