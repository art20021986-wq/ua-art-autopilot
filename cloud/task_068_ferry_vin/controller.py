#!/usr/bin/env python3
"""Upload, install, restart and verify UA-CARDS-FERRY-VIN-001 V1.1."""
...
STATE_ROOT = REMOTE
FILES = {
    "task068_ferry_vin_repair.py": HERE / "repair_remote.py",
    "task068_ferry_vin_postcheck.py": HERE / "postcheck_remote.py",
}
...
INSTALL_COMMAND = "cd %s && python3.10 task068_ferry_vin_repair.py" % REMOTE
...
class API:
    ...
    def request(self, method, url, data=None, headers=None, allowed=(200,)):
        request_headers = {
            ...
            "User-Agent": "ua-art-task068-ferry-vin/1",
        }
    ...
    def upload(self, path: str, data: bytes) -> None:
        boundary = "----uaart-task068-" + uuid.uuid4().hex
...
def validate_install(value: dict) -> None:
    ...
    for card in cards:
        ...
        for item in roots.values():
            if (item.get("stage_anchor_count") != 1
                    or item.get("diagnostics_links") != 1
        ...
        if card.get("id") in {"UA-0009", "UA-0010"} and any(
...
def main() -> int:
    ...
    try:
        ...
        source_install = api.run_remote(
            SOURCE_COMMAND,
            "task068 install final ferry VIN media writer sources only",
            SOURCE_RECEIPT,
        )
        ...
        install = api.run_remote(
            INSTALL_COMMAND,
            "task068 install permanent ferry VIN diagnostics media contract",
            INSTALL_RECEIPT,
        )
        ...
        postcheck = api.run_remote(
            POSTCHECK_COMMAND,
            "task068 verify cards, catalogs and both Telegram bots",
            POSTCHECK_RECEIPT,
        )
        ...
        delayed = api.run_remote(
            POSTCHECK_COMMAND,
            "task068 delayed permanence check after background writers",
            POSTCHECK_RECEIPT,
        )
        ...
        if api is not None and install and install.get("status") == "PASS" and install.get("changed_paths"):
            try:
                rollback = api.run_remote(
                    ROLLBACK_COMMAND,
                    "task068 rollback failed ferry VIN deployment",
                    ROLLBACK_RECEIPT,
                )
