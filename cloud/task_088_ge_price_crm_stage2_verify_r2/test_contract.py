...
try:
    from price_parser import parse_sale_price_message as _parse_sale_price
except ImportError:
    _parse_sale_price = None
...
class FakeAPI:
    def __init__(self, drift=False, missing_parser=False):
        self.get_count = self.response_body_bytes = 0
        self.paths, self.drift, self.missing_parser = [], drift, missing_parser
    def read(self, path):
        self.get_count += 1
        self.paths.append(path)
        if self.missing_parser and path == c.SOURCE_PATHS[2]:
            raise c.DiscoveryError("REMOTE_HTTP_404")
        data = {c.STAGE1_REMOTE_RECEIPT: remote_receipt(), c.SOURCE_PATHS[0]: FIXTURE.encode(),
                c.SOURCE_PATHS[1]: DB, c.SOURCE_PATHS[2]: PARSER,
                c.SOURCE_PATHS[3]: b'SPLIT_RE = "[0-9]+"\ndef main():\n    return None\n'}[path]
...
class ContractTests(unittest.TestCase):
    ...
    def test_complete_get_evidence_never_claims_installation_or_acceptance(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(api.get_count, 10)
    ...
    def test_concurrent_source_drift_is_reported_and_candidate_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            env, api = self.setup_tree(root), FakeAPI(drift=True)
            result = c.execute(env, root=root, api=api)
            evidence = json.loads((root / c.EVIDENCE_REL).read_text())
            self.assertEqual(result["source_snapshot_consistency"], "PARTIAL")
            self.assertEqual(result["candidate_verification"], "REFUSED")
            self.assertEqual(evidence["candidate_verification"]["stop_reason"], "SOURCE_CHANGED_DURING_VERIFICATION")
            self.assertEqual(evidence["path_status"][c.SOURCE_PATHS[0]]["snapshot_consistency"], "DRIFT")

    def test_missing_parser_preserves_full_candidate_proof_and_other_helpers(self):
        module = local_patcher()
        proxy = types.SimpleNamespace(TARGETS=module.TARGETS, HELPERS=module.HELPERS,
            build_candidate=lambda value: module.build_candidate(value, expected_sha256=module.digest(FIXTURE)))
        original_verify = c.verify_full_candidate
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            env, api = self.setup_tree(root), FakeAPI(missing_parser=True)
            with mock.patch.object(c, "verify_full_candidate", side_effect=lambda payload, root: original_verify(payload, root=root, patcher=proxy)):
                receipt = c.execute(env, root=root, api=api)
            evidence = json.loads((root / c.EVIDENCE_REL).read_text())
            self.assertEqual(api.get_count, 10)
            self.assertEqual(receipt["candidate_verification"], "PASS")
            self.assertEqual(evidence["path_status"][c.SOURCE_PATHS[2]]["first_get"]["error"], "REMOTE_HTTP_404")
            self.assertEqual(evidence["path_status"][c.SOURCE_PATHS[0]]["snapshot_consistency"], "PASS")
            self.assertIn("Soedinenie", evidence["sources"][c.SOURCE_PATHS[1]]["selected_functions"])
            self.assertEqual(evidence["candidate_verification"]["before_sha256"], c.sha(FIXTURE.encode()))
            shape = evidence["parser_import_shape"]["imports"][0]
            self.assertTrue(shape["try_except_guarded"])
            self.assertEqual(shape["caught_exceptions"], ["ImportError"])
            self.assertEqual(shape["none_fallback_aliases"], ["_parse_sale_price"])

    def test_startup_constants_have_selected_no_secret_export(self):
        source = b'SPLIT_RE = "[0-9]+"\nMUSOR = ["tmp"]\nSVYATOE = {"api_key": "must-omit"}\n'
        result = c.inspect_source(source, c.SOURCE_PATHS[3])
        values = {item["startup"]["names"][0]: item["startup"] for item in result["field_constants"]}
        self.assertTrue(values["SPLIT_RE"]["source_exported"])
        self.assertFalse(values["SVYATOE"]["source_exported"])
        self.assertNotIn("must-omit", json.dumps(result))
