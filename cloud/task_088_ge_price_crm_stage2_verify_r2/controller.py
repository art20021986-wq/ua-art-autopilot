#!/usr/bin/env python3
"""TASK088 full-source verification: fixed GETs, in-memory patch, no live execution.
...
TASK_ID = "TASK088-GE-PRICE-CRM-STAGE2-VERIFY-R2"
STAGE1_ID = "TASK088-GE-PRICE-CRM-STAGE1"
ROOT = pathlib.Path(__file__).resolve().parents[2]
PACKAGE_REL = "cloud/task_088_ge_price_crm_stage2_verify_r2"
...
SOURCE_PATHS = tuple("/home/Carix/" + name for name in (
    "cars_ui.py", "db.py", "price_parser.py", "start_safe.py"))
...
BACKUP_PATHS = ()  # No database or backup URLs are requested.
...
SELECTED_FUNCTIONS = frozenset((
    "remember_price", "Soedinenie", "log_action", "connect", "db",
    "update_card_field", "parse_sale_price_message",
    "_ua004_stage_reconcile_job", "_ua004_sync_current_stage", "_ua004_source_signature",
    "main", "pochinit_imena", "db_clean_split"))
STARTUP_CONSTANTS = frozenset(("SPLIT_RE", "SVYATOE", "MUSOR", "MUSOR_KONCY"))
STAGE1_PINNED_SHA256 = "f8a2325b036010e2d5f35b277fe3039eed7d974c708f8fd73dc61318a8779d82"
PATCHER_REL = PACKAGE_REL + "/patcher.py"
RUNTIME_REL = PACKAGE_REL + "/runtime.py"
DEPENDENCY_PATHS = frozenset((PATCHER_REL, RUNTIME_REL))
...
def selected_startup_constant(source, node):
    if not isinstance(node, (ast.Assign, ast.AnnAssign)):
        return None
    targets = node.targets if isinstance(node, ast.Assign) else [node.target]
    names = [item.id for item in targets if isinstance(item, ast.Name) and item.id in STARTUP_CONSTANTS]
    if not names:
        return None
    segment = ast.get_source_segment(source, node)
    if segment is None:
        return {"names": names, "source_exported": False, "omission_reason": "SEGMENT_MISSING"}
    reason = contains_sensitive_source(segment, node)
    if len(segment.encode()) > MAX_FUNCTION_BYTES:
        reason = "EXPORT_SIZE_LIMIT"
    result = {"names": names, "sha256": sha(segment.encode()), "source_exported": not bool(reason)}
    result.update({"omission_reason": reason} if reason else {"source": segment})
    return result


def parser_import_shape(payload):
    """Report optional import syntax without executing it or claiming runtime support."""
    tree = ast.parse(payload.decode("utf-8-sig"))
    parents = {child: node for node in ast.walk(tree) for child in ast.iter_child_nodes(node)}
    imports = []
    for node in ast.walk(tree):
        if not (isinstance(node, ast.ImportFrom) and node.module == "price_parser"):
            continue
        item = {"module": "price_parser", "line": node.lineno,
                "names": [{"name": safe_symbol(n.name), "alias": safe_symbol(n.asname) if n.asname else None}
                          for n in node.names], "try_except_guarded": False,
                "caught_exceptions": [], "none_fallback_aliases": []}
        child = node
        while child in parents:
            parent = parents[child]
            if isinstance(parent, ast.Try) and child in parent.body:
                item["try_except_guarded"] = True
                aliases = {n.asname or n.name for n in node.names}
                for handler in parent.handlers:
                    kinds = handler.type.elts if isinstance(handler.type, ast.Tuple) else [handler.type]
                    item["caught_exceptions"].extend(symbol(kind) if kind else "BARE_EXCEPT" for kind in kinds)
                    for statement in handler.body:
                        if isinstance(statement, ast.Assign) and isinstance(statement.value, ast.Constant) and statement.value.value is None:
                            item["none_fallback_aliases"].extend(target.id for target in statement.targets
                                if isinstance(target, ast.Name) and target.id in aliases)
                break
            child = parent
        imports.append(item)
    return {"scope": "STATIC_IMPORT_SYNTAX_ONLY", "imports": imports,
            "parser_code_executed": False, "runtime_parser_availability": "NOT_VERIFIED"}


class RedactSettings(ast.NodeTransformer):
    """Produce analysis-only syntax while removing setting values and comments."""
    SAFE_LITERALS = PRICE_FIELDS | PRICE_LABELS | frozenset((
        "car_wait", "card_id", "field", "client", "text", "voice",
        "id", "price_history", "updated_at", "price", "cars",
    ))
    @staticmethod
    def hidden():
        return ast.Constant(value="[REDACTED_SETTING]")

    def visit_Constant(self, node):
        if isinstance(node.value, (str, bytes)):
            text = node.value.decode("utf-8", errors="replace") if isinstance(node.value, bytes) else node.value
            if TOKEN_PATTERN.search(text) or text not in self.SAFE_LITERALS:
                return ast.copy_location(self.hidden(), node)
        elif node.value is not None and not isinstance(node.value, bool):
            return ast.copy_location(self.hidden(), node)
        return node

    def visit_Assign(self, node):
        self.generic_visit(node)
        if any(SECRET_NAME.search(part.id if isinstance(part, ast.Name) else part.attr if isinstance(part, ast.Attribute) else str(part.value) if isinstance(part, ast.Constant) else "") for target in node.targets for part in ast.walk(target)):
            node.value = self.hidden()
        return node

    def visit_AnnAssign(self, node):
        self.generic_visit(node)
        if any(SECRET_NAME.search(part.id) for part in ast.walk(node.target) if isinstance(part, ast.Name)):
            node.value = self.hidden()
        return node
    ...
    def visit_NamedExpr(self, node):
        self.generic_visit(node)
        if isinstance(node.target, ast.Name) and SECRET_NAME.search(node.target.id):
            node.value = self.hidden()
        return node

    def visit_keyword(self, node):
        self.generic_visit(node)
        if SECRET_NAME.search(node.arg or ""):
            node.value = self.hidden()
        return node
    ...
    def visit_Dict(self, node):
        ...
        for i, key in enumerate(node.keys):
            if isinstance(key, ast.Constant) and isinstance(key.value, str) and SECRET_NAME.search(key.value):
                node.values[i] = self.hidden()
        return node

    def visit_arguments(self, node):
        self.generic_visit(node)
        positional = [*node.posonlyargs, *node.args]
        defaulted = positional[-len(node.defaults):] if node.defaults else []
        node.defaults = [self.hidden() if SECRET_NAME.search(arg.arg) else value for arg, value in zip(defaulted, node.defaults)]
        node.kw_defaults = [self.hidden() if value is not None and SECRET_NAME.search(arg.arg) else value for arg, value in zip(node.kwonlyargs, node.kw_defaults)]
        return node


def redacted_analysis_source(segment):
    tree = ast.parse(segment)
    tree = ast.fix_missing_locations(RedactSettings().visit(tree))
    for item in ast.walk(tree):
        if isinstance(item, ast.Constant) and item.value is not None and not isinstance(item.value, bool):
            value = item.value.decode("utf-8", errors="replace") if isinstance(item.value, bytes) else item.value
            if value != "[REDACTED_SETTING]" and value not in RedactSettings.SAFE_LITERALS:
                raise DiscoveryError("REDACTION_RESIDUAL_LITERAL")
    result = ast.unparse(tree)
    if len(result.encode("utf-8")) > MAX_FUNCTION_BYTES:
        return None
    return result
    ...
    for index, node in enumerate(tree.body):
        ...
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            ...
            if node.name in SELECTED_FUNCTIONS:
                ...
                reason = contains_sensitive_source(segment, node)
                if node.name == "catch_message":
                    reason = "ANALYSIS_ONLY_HANDLER"
                ...
                if reason:
                    ...
                    if node.name == "catch_message" and reason != "EXPORT_SIZE_LIMIT":
                        sanitized = redacted_analysis_source(segment)
                        if sanitized is not None and exported + len(sanitized.encode("utf-8")) <= MAX_EXPORTED_SOURCE_BYTES:
                            details.update({"redacted_analysis_source": sanitized,
                                            "redaction_applied": True,
                                            "analysis_only_not_executable_source": True})
                            exported += len(sanitized.encode("utf-8"))
                else:
                    details.update({"source_exported": True, "source": segment,
                                    "source_scope": "EXACT_SELECTED_HELPER_WITHOUT_DECORATORS"})
                    exported += count
        ...
        startup = selected_startup_constant(source, node)
        if startup is not None:
            constants.append({"order": index, "line": node.lineno, "startup": startup})
...
def read_local_bytes(root, relative, limit):
    if relative not in DEPENDENCY_PATHS | {STAGE1_RECEIPT_REL}:
        raise DiscoveryError("LOCAL_BYTES_SCOPE")
    path = root
    for component in pathlib.PurePosixPath(relative).parts:
        path = path / component
        if path.is_symlink():
...
def independent_ast_proof(before, after, targets, helpers):
    """Check unchanged syntax independently of patcher's own assertions."""
    ...
    excluded = set(targets) | set(helpers)
    before_other = [dump(node) for node in before_tree.body if not (is_function(node) and node.name in excluded)]
    after_other = [dump(node) for node in after_tree.body if not (is_function(node) and node.name in excluded)]
    if before_other != after_other:
        raise DiscoveryError("UNRELATED_AST_CHANGED")
    functions = {}
    for name in targets:
        old = [node for node in before_tree.body if is_function(node) and node.name == name]
        new = [node for node in after_tree.body if is_function(node) and node.name == name]
        if len(old) != 1 or len(new) != 1:
            raise DiscoveryError("TARGET_COUNT_DRIFT")
        old, new = old[0], new[0]
        if dump(old.args) != dump(new.args) or [dump(d) for d in old.decorator_list] != [dump(d) for d in new.decorator_list]:
            raise DiscoveryError("TARGET_SIGNATURE_DRIFT")
        old_body, new_body = [dump(node) for node in old.body], [dump(node) for node in new.body]
        additions = [i for i in range(len(new_body)) if new_body[:i] + new_body[i + 1:] == old_body]
        if len(new_body) != len(old_body) + 1 or len(additions) != 1:
            raise DiscoveryError("TARGET_BODY_DRIFT")
        functions[name] = {"before_ast_sha256": sha(dump(old).encode()),
                           "after_ast_sha256": sha(dump(new).encode()),
                           "original_statements_preserved": True,
                           "added_statement_index": additions[0],
                           "added_statement_type": type(new.body[additions[0]]).__name__}
    find = lambda name: next(node for node in after_tree.body if is_function(node) and node.name == name)
    catch = find("catch_message")
    explicit = [i for i, node in enumerate(catch.body) if isinstance(node, ast.If) and any(
        isinstance(item, ast.Call) and isinstance(item.func, ast.Name) and item.func.id == "_task088_apply_selected_price"
        for item in ast.walk(node))]
    general = [i for i, node in enumerate(catch.body) if isinstance(node, ast.If) and ast.unparse(node.test) == "not wait"]
    if len(explicit) != 1 or len(general) != 1 or explicit[0] >= general[0]:
        raise DiscoveryError("EXPLICIT_PRICE_ROUTE_ORDER")
    branch = catch.body[explicit[0]]
    helper_calls = [node for node in ast.walk(branch) if isinstance(node, ast.Call)
                    and isinstance(node.func, ast.Name) and node.func.id == "_task088_apply_selected_price"]
    if len(helper_calls) != 1 or len(helper_calls[0].args) != 4 or ast.unparse(helper_calls[0].args[1]) != "wait['field']":
        raise DiscoveryError("SELECTED_PRICE_FIELD_NOT_PRESERVED")
    if not any(isinstance(node, ast.Raise) and isinstance(node.exc, ast.Name)
               and node.exc.id == "ApplicationHandlerStop" for node in ast.walk(branch)):
        raise DiscoveryError("EXPLICIT_PRICE_ROUTE_FALLTHROUGH")
    actual_helpers = {node.name for node in after_tree.body if is_function(node) and node.name in helpers}
    if actual_helpers != set(helpers):
        raise DiscoveryError("HELPER_SET_DRIFT")
    return {"unrelated_top_level_ast": "PASS", "unrelated_node_count": len(before_other),
            "unrelated_ast_sha256": sha(json.dumps(before_other, separators=(",", ":")).encode()),
            "target_statements": functions,
            "explicit_price_branch_before_general_route": "PASS",
            "selected_field_argument_preserved": "PASS",
            "handler_stop_after_explicit_price_route": "PASS",
            "runtime_handler_binding": "NOT_VERIFIED"}


def verify_full_candidate(payload, *, root, patcher=None):
    """Only local reviewed patcher runs; downloaded module is never executed."""
    result = {"before_sha256": sha(payload), "input_scope": "FULL_ORIGINAL_REMOTE_FILE_BYTES",
              "full_source_exported": False, "candidate_source_exported": False,
              "downloaded_code_executed": False, "installation": "NOT_PERFORMED",
              "live_acceptance": "NOT_PERFORMED"}
    try:
        source = payload.decode("utf-8")
        compile(source, "remote_source_static_check.py", "exec")
        if patcher is None:
            path = root / PATCHER_REL
            spec = importlib.util.spec_from_file_location("task088_local_verified_patcher", path)
            patcher = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(patcher)  # Pinned repository code, not downloaded code.
        candidate, _ = patcher.build_candidate(source)
        compile(candidate, "candidate_static_check.py", "exec")
        proof = independent_ast_proof(source, candidate, patcher.TARGETS, patcher.HELPERS)
        result.update({"status": "PASS", "after_sha256": sha(candidate.encode("utf-8")),
                       "before_bytes": len(payload), "after_bytes": len(candidate.encode("utf-8")),
                       "original_compile": "PASS", "candidate_compile": "PASS", "ast_and_routing": proof})
    except Exception as exc:
        # Preserve safe helper evidence even when source fingerprint/anchors drift.
        reason = str(exc)
        reason = reason if re.fullmatch(r"[A-Z0-9_]+(?::[A-Za-z_0-9]+)?", reason) else "CANDIDATE_VERIFICATION_ERROR"
        result.update({"status": "REFUSED", "stop_reason": reason,
                       "candidate_compile": "NOT_ACCEPTED"})
    return result
...
def validate_identity(env, root):
    ...
    if sha(prior_raw) != STAGE1_PINNED_SHA256:
        raise DiscoveryError("STAGE1_IMMUTABLE_HASH_MISMATCH")
    if set(execution.get("dependency_paths", [])) != DEPENDENCY_PATHS:
        raise DiscoveryError("DEPENDENCY_SCOPE")
    hashes = execution.get("file_sha256", {})
    for relative in DEPENDENCY_PATHS:
        data = read_local_bytes(root, relative, MAX_SOURCE_BYTES)
        if sha(data) != hashes.get(relative):
            raise DiscoveryError("DEPENDENCY_SHA_MISMATCH")
...
def safe_read(api, path):
    try:
        payload = api.read(path)
        return payload, {"status": "RECEIVED", "sha256": sha(payload), "bytes": len(payload)}
    except DiscoveryError as exc:
        code = str(exc) if re.fullmatch(r"[A-Z0-9_]+", str(exc)) else "REMOTE_READ_FAILURE"
        return None, {"status": "FAILED", "error": code}
...
def execute(env: Mapping[str, str], *, root=None, api=None):
    ...
    started = now()
    paths = (STAGE1_REMOTE_RECEIPT, *SOURCE_PATHS)
    originals, path_status = {}, {}
    for path in paths:
        payload, status = safe_read(api, path)
        path_status[path] = {"first_get": status}
    ...
    for path in SOURCE_PATHS:
        if path not in originals:
            continue
        try:
            sources[path] = inspect_source(originals[path], path)
            path_status[path]["syntax"] = "PASS"
        except DiscoveryError as exc:
            path_status[path]["syntax"] = "FAILED"
            path_status[path]["syntax_error"] = str(exc)
    stage1 = {"path": STAGE1_REMOTE_RECEIPT, "status": "NOT_AVAILABLE",
              "live_schema_verification": "NOT_PERFORMED"}
    if STAGE1_REMOTE_RECEIPT in originals:
        try:
            stage1 = remote_stage1_summary(originals[STAGE1_REMOTE_RECEIPT])
        except DiscoveryError as exc:
            stage1 = {"path": STAGE1_REMOTE_RECEIPT, "status": "NOT_ACCEPTED", "error": str(exc)}
    helper_presence = {name: [path for path, value in sources.items() if name in value["selected_functions"]]
                       for name in sorted(SELECTED_FUNCTIONS)}
    candidate = {"status": "NOT_PERFORMED", "stop_reason": "CARS_UI_NOT_AVAILABLE_OR_INVALID",
                 "installation": "NOT_PERFORMED", "live_acceptance": "NOT_PERFORMED"}
    import_shape = {"status": "NOT_AVAILABLE"}
    if SOURCE_PATHS[0] in sources:
        candidate = verify_full_candidate(originals[SOURCE_PATHS[0]], root=root)
        import_shape = parser_import_shape(originals[SOURCE_PATHS[0]])
    for path in paths:
        current, status = safe_read(api, path)
        path_status[path]["second_get"] = status
        if path in originals and current is not None:
            consistency = "PASS" if sha(current) == sha(originals[path]) else "DRIFT"
        else:
            consistency = "NOT_VERIFIED"
        path_status[path]["snapshot_consistency"] = consistency
        if path == SOURCE_PATHS[0] and consistency == "DRIFT":
            candidate.update({"status": "REFUSED", "stop_reason": "SOURCE_CHANGED_DURING_VERIFICATION"})
    if sha(read_local_bytes(root, STAGE1_RECEIPT_REL, MAX_RECEIPT_BYTES)) != stage1_sha:
        raise DiscoveryError("STAGE1_DURABLE_CHANGED_DURING_DISCOVERY")
    del originals
    finished = now()
    consistency = "PASS" if all(value["snapshot_consistency"] == "PASS" for value in path_status.values()) else "PARTIAL"
    evidence = {"task_id": TASK_ID, "scope": "GET_ONLY_FULL_SOURCE_VERIFICATION",
                "status": "COMPLETED" if consistency == "PASS" else "COMPLETED_WITH_PATH_ERRORS",
                "run_id": run_id, "request_sha256": request_sha, "started_at": started, "finished_at": finished,
                ...
                "stage1_remote_receipt": stage1, "backup_integrity": "NOT_VERIFIED_GET_ONLY",
                "path_status": path_status, "sources": sources, "selected_helper_presence": helper_presence,
                "candidate_verification": candidate, "source_snapshot_consistency": consistency,
                "parser_import_shape": import_shape, "parser_verification": "STATIC_AST_ONLY_NOT_EXECUTED",
    ...
    receipt = {"task_id": TASK_ID, "status": "FINISHED", "task_class": "STANDARD",
               "target_environment": "shadow", "live_source_environment": "production",
               "tests": "PASS", "tests_scope": "GET_SCOPE_IMMUTABLE_PREREQUISITE_AND_SAFE_PARTIAL_EVIDENCE",
               "candidate_verification": candidate["status"], "source_snapshot_consistency": consistency,
               "remote_path_errors": [path for path, value in path_status.items() if value["snapshot_consistency"] != "PASS"],
               ...
               "run_id": run_id, "scope": "GET_ONLY_FULL_SOURCE_VERIFICATION", "remote_write_count": 0,
               "stage2_status": "NOT_PERFORMED", "stage2_acceptance": "NOT_PERFORMED",
               "crm_price_acceptance": "NOT_PERFORMED", "finished_at": finished,
               "completion_meaning": "READ_ONLY_VERIFICATION_ONLY_NO_INSTALLATION",
