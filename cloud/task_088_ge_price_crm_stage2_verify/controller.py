...
TASK_ID = "TASK088-GE-PRICE-CRM-STAGE2-VERIFY"
STAGE1_ID = "TASK088-GE-PRICE-CRM-STAGE1"
ROOT = pathlib.Path(__file__).resolve().parents[2]
PACKAGE_REL = "cloud/task_088_ge_price_crm_stage2_verify"
...
SOURCE_PATHS = tuple("/home/Carix/" + name for name in (
    "cars_ui.py", "db.py", "price_parser.py"))
...
def execute(env: Mapping[str, str], *, root=None, api=None):
    ...
    originals = {path: api.read(path) for path in SOURCE_PATHS}
    sources = {path: inspect_source(payload, path) for path, payload in originals.items()}
    helper_presence = {name: [path for path in SOURCE_PATHS
                             if name in sources[path]["selected_functions"]]
                       for name in sorted(SELECTED_FUNCTIONS)}
    candidate = verify_full_candidate(originals[SOURCE_PATHS[0]], root=root)
    # Re-reading proves stability only within this verification window.
    ...
    del originals
    finished = now()
    evidence = {"task_id": TASK_ID, "scope": "GET_ONLY_FULL_SOURCE_VERIFICATION", "status": "COMPLETED",
                ...
                "sources": sources, "selected_helper_presence": helper_presence,
                "candidate_verification": candidate, "source_snapshot_consistency": "PASS",
                "parser_verification": "STATIC_AST_ONLY_NOT_EXECUTED",
    ...
    receipt = {"task_id": TASK_ID, "status": "FINISHED", "task_class": "STANDARD",
               "target_environment": "shadow", "live_source_environment": "production",
               "tests": "PASS", "tests_scope": "GET_SCOPE_IMMUTABLE_PREREQUISITE_SOURCE_EXTRACTION_AND_CONSISTENCY",
               "candidate_verification": candidate["status"],
