...
_FETCH_FAILURE_CACHE: dict[str, str] = {}
...
def match_profile(car: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        cc = 0
    for profile in profile_library.PROFILES:
        if not any(vin.startswith(str(prefix)) for prefix in profile["vin_prefixes"]):
            continue
        if brand and not any(_norm(token) in brand for token in profile.get("brand_tokens") or ()):
            continue
        if model and not any(_norm(token) in model for token in profile.get("model_tokens") or ()):
            continue
        if fuel and not any(_norm(token) in fuel for token in profile.get("fuel_tokens") or ()):
            continue
        lower, upper = profile.get("cc_range") or (0, 99999)
        if cc and not int(lower) <= cc <= int(upper):
            continue
        return profile
    return None
...
def _open_bytes(
    ...
    *,
    ...
    cacheable = (
        method == "GET" and data is None and opener is urllib.request.urlopen
        and initial_domain != DISCOVERY_DOMAIN
    )
    if cacheable and url in _BODY_CACHE:
        return _BODY_CACHE[url]
    if cacheable and url in _FETCH_FAILURE_CACHE:
        raise SourcePolicyError(_FETCH_FAILURE_CACHE[url])
    request_headers = {
    ...
    try:
        ...
        if cacheable:
            _FETCH_FAILURE_CACHE[url] = str(exc)
        raise
    except urllib.error.HTTPError as exc:
        if cacheable:
            _FETCH_FAILURE_CACHE[url] = "HTTP_%d" % exc.code
        raise SourcePolicyError("HTTP_%d" % exc.code) from exc
    except Exception as exc:
        if cacheable:
            _FETCH_FAILURE_CACHE[url] = "NETWORK_%s" % type(exc).__name__
...
def vpic_identity_matches(car: dict[str, Any], identity: dict[str, Any]) -> bool:
    ...
    if model and tokens["model"] and not _any_token(model, tokens["model"]):
        return False
    if year and tokens["year"] and year not in tokens["year"]:
        return False
    return True
...
def deduplicate(facts: Iterable[Fact]) -> list[dict[str, Any]]:
    ...
    for key in sorted(grouped):
        ...
        winning = max(
            by_value.values(),
            key=lambda items: (
                max(SOURCE_PRIORITY.get(item.source_domain, 0) for item in items),
        ...
        ordered = sorted(
            winning,
            key=lambda item: (SOURCE_PRIORITY.get(item.source_domain, 0), item.confidence),
            reverse=True,
        )
        best = ordered[0]
...
def enrich(
    ...
    *,
    ...
    for domain in SOURCE_DOMAINS[1:]:
        ...
        sources[domain] = {
            "status": "PASS" if accepted else "CURATED_PASS" if curated_count else "NO_CONFIDENT_MATCH",
            "facts": len(accepted) + curated_count,
    ...
    detailed_domains = [
        ...
        if sources.get(domain, {}).get("status") in {"PASS", "CURATED_PASS"}
    ]
...
def audit_sources(*, opener: Callable[..., Any] = urllib.request.urlopen) -> dict[str, Any]:
    ...
    return {
        "policy_version": POLICY_VERSION,
        "audited_on": profile_library.AUDIT_DATE,
