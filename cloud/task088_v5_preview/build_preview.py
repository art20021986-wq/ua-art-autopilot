...
from routing_proof import prove_legacy_home_redirect, prove_unserved_site_prefix
from viewport_harness import HARNESS_ROUTE, HTML_MIME, render as render_viewport, specification as viewport_specification
...
def build(snapshot, output, source_origin, routing_evidence, asset_root=None, site_asset_root=None):
    ...
    unserved_site_proof = prove_unserved_site_prefix(captured_file,routes,sha(route_raw))
    source_checks = []
    ...
    for folder in ('video','site'):
        for row in rows:
            ...
            target = payload if folder == 'video' else offline
            target[name] = candidate.encode()
            pages.append(dict(path='/' + name, kind='CARD',
                classification='SERVED_CANDIDATE' if folder=='video' else 'OFFLINE_PROTECTED_ARTIFACT',
                candidate_path=('public/' if folder=='video' else 'offline/')+name, **evidence))
        name = folder + '/katalog.html'
        candidate, evidence = migrate_catalog(captured_file(name).decode(), rows)
        target = payload if folder == 'video' else offline
        target[name] = candidate.encode()
        pages.append(dict(path='/' + name, kind='CATALOG',
            classification='SERVED_CANDIDATE' if folder=='video' else 'OFFLINE_PROTECTED_ARTIFACT',
            candidate_path=('public/' if folder=='video' else 'offline/')+name, **evidence))
    candidate, evidence = migrate_home(captured_file('video/index.html').decode(), rows)
    payload['video/index.html'] = candidate.encode()
    pages.append(dict(path='/video/index.html', kind='HOME', classification='SERVED_CANDIDATE',
                      candidate_path='public/video/index.html', **evidence))
    served_pages = [page for page in pages if page['classification']=='SERVED_CANDIDATE']
    ...
    auxiliary_allowed = {folder + '/' + name for folder in ('video',)
    ...
    while pending:
        ...
        if name.startswith('site/'):
            # A real served page pointing at /site/ is still a broken public
            # dependency, even if the offline source file exists on disk.
            missing.append({'path':'/'+name,'reason':'UNSERVED_PRODUCTION_ASSET_ROUTE'});continue
    ...
    matrix = [{'path':page['path'],'kind':page['kind'],'source_diff':'PASS_PRICE_ONLY_OR_UNCHANGED_HOME',
               'browser':[{ 'language':lang,'viewport':size,'status':'NOT_RUN'}
                          for lang in ('RU','UA','GE') for size in ('desktop','mobile')]} for page in served_pages]
    ...
    # This separate Preview-only document wraps unchanged candidate bytes. It
    # never becomes a price surface or a successful browser-matrix observation.
    viewport_spec = viewport_specification(files)
    viewport_html = render_viewport(files)
    payload[HARNESS_ROUTE[1:]] = viewport_html
    files[HARNESS_ROUTE] = {'storage':'bundle','path':'public'+HARNESS_ROUTE,
        'sha256':sha(viewport_html),'bytes':len(viewport_html),'content_type':HTML_MIME,
    ...
    provenance = {'contract':CONTRACT,'source_capture_sha256':sha(captured_raw),
        ...
        'coverage':{'protected_price_sources':len(pages),'served_price_pages':len(served_pages),
                    'offline_protected_price_artifacts':len(offline),'browser_checks':len(served_pages)*6},
        ...
        'unserved_site_prefix_proof':unserved_site_proof,
        'viewport_harness':viewport_spec,
    ...
    for name, raw in offline.items(): write_new(output, 'offline/' + name, raw)
    ...
    return {'output':str(output),'manifest_sha256':sha(encoded(manifest)),'pages':len(pages),
            'served_pages':len(served_pages),'offline_protected_pages':len(offline),
            'browser_checks':len(served_pages)*6,
            'pinned_assets':len(files)-len(served_pages)-len(linked_pages)-1,'missing_assets':len(missing),
            'viewport_harness':HARNESS_ROUTE,
