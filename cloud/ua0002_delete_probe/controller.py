...
TASK_ID = 'UA-ART-UA0002-DELETE-PROBE-001'
PACKAGE = 'cloud/ua0002_delete_probe'
...
SOURCE_NAMES = (
    'cars_ui.py', 'yadro.py', 'stranica.py', 'db.py', 'publication_fence.py',
    'visibility_lifecycle.py', 'publish_transaction_guard.py', 'publikaciya.py',
    'ua_site_counters.py', 'uaart_price_sync_binding.py', 'uaart_price_sync_runtime.py',
    'uaart_price_sync_outbox.py', 'ua_spec_permanent.py', 'ua_spec84_runtime.py',
    'ua_spec84_schedule.py', 'start_safe.py', 'run_all.py',
)
SOURCE_PATHS = tuple('/home/Carix/' + name for name in SOURCE_NAMES)
PAGE_PATHS = tuple('/home/Carix/' + folder + '/' + name
    for folder in ('video', 'site')
    for name in ('UA-0002.html', 'UA-0002-diag.html', 'katalog.html', 'index.html')) + (
    '/home/Carix/video/sitemap.xml',)
FILE_PATHS = SOURCE_PATHS + PAGE_PATHS
ENDPOINTS = tuple('files/path' + path for path in FILE_PATHS) + ('always_on/',)
...
KNOWN_COMMAND_MODULES = ('start_safe.py', 'run_all.py', 'cars_ui.py', 'yadro.py', 'stranica.py',
    ...
    'uaart_price_sync_outbox.py', 'uaart_price_sync_runtime.py')
...
class ReadOnlyAPI:
    ...
    def read(self, endpoint):
        ...
        request = urllib.request.Request(url, method='GET', headers={
            ...
            'User-Agent': 'uaart-ua0002-get-only-probe/1'})
        try:
...
def inspect_source(payload, path):
    ...
    for item in tree.body:
        if isinstance(item, (ast.Import, ast.ImportFrom)):
            ...
            if isinstance(item, ast.ClassDef) or item.name not in SELECTED_DEFINITIONS:
...
def execute(env, *, root=None, api=None):
    ...
    evidence = {'task_id': TASK_ID, 'status': 'PROBE_COMPLETED', 'request_sha256': request_sha,
        ...
        'source_facts': sources, 'page_facts': pages, 'always_on': tasks,
