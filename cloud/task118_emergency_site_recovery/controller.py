...
TASK_ID = "TASK118-EMERGENCY-SITE-RECOVERY"
CONTRACT = "UA-ART-EMERGENCY-WEBAPP-RECOVERY-001-V1.0"
...
EVIDENCE_REL = "cloud/task118_emergency_site_recovery/evidence.json"
...
def validate_page(url: str, status: int, final_url: str,
    ...
    return {"url": url, "final_url": final_url, "http": status,
            "bytes": len(body), "sha256": sha(body)}


def probe(url: str, timeout: int = 25) -> dict[str, Any]:
    request = urllib.request.Request(
        url, headers={"User-Agent": "UA-ART-emergency-recovery-health/1",
...
def probe_round() -> list[dict[str, Any]]:
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(CORE_URLS)) as pool:
        return list(pool.map(probe, CORE_URLS))
...
def wait_for_consecutive(required_passes: int = 3,
    ...
    deadline, consecutive = time.monotonic() + timeout_seconds, 0
    ...
    while time.monotonic() < deadline:
        try:
            ...
            attempts.append({"checked_at": now(), "status": "PASS",
                             "consecutive": consecutive, "checks": checks})
