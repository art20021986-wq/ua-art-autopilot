#!/usr/bin/env python3
"""Install, restart, verify, and if necessary roll back task_065."""
...
FILES = {
    "photo_fastpath_installer.py": HERE / "photo_fastpath_installer.py",
    "photo_fastpath_postcheck.py": HERE / "photo_fastpath_postcheck.py",
}
...
INSTALL_COMMAND = "cd %s && python3.10 photo_fastpath_installer.py" % REMOTE
POSTCHECK_COMMAND = "cd %s && python3.10 photo_fastpath_postcheck.py" % REMOTE
ROLLBACK_COMMAND = "cd %s && python3.10 photo_fastpath_installer.py --rollback" % REMOTE
...
class API:
    ...
    def request(self, method, url, data=None, headers=None, allowed=(200,)):
        request_headers = {
            ...
            "User-Agent": "ua-art-task065-photo-fastpath/1",
        }
...
def main() -> int:
    ...
    try:
        ...
        install = api.run_remote(INSTALL_COMMAND, "task065 install CRM photo fastpath",
                                 INSTALL_RECEIPT)
        ...
        time.sleep(10)
        postcheck = api.run_remote(POSTCHECK_COMMAND, "task065 verify CRM photo fastpath",
                                   POSTCHECK_RECEIPT)
        ...
        if api is not None and install and install.get("status") == "PASS" and not install.get("already_applied"):
            try:
                rollback = api.run_remote(ROLLBACK_COMMAND, "task065 rollback CRM photo fastpath",
                                          ROLLBACK_RECEIPT)
