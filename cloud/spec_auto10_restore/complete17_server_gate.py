#!/usr/bin/env python3
"""PA launcher for combined17 isolated rehearsal; no install or restart."""
...
STAGE = Path("/home/Carix/spec_complete17_gate_20260909")
SOURCE = Path("/home/Carix")
MANIFEST_SHA = "12afce821b784771a2a8a0415cc289f5e713d53aabedf0ed3e2747a8c4d4e136"
RUNNERS = {"complete17_cycle.py", "complete17_server_gate.py", "complete15_cycle.py", "full_publisher_rehearsal.py"}
...
def main():
    ...
    if STAGE.is_symlink() or STAGE.resolve() != STAGE or bundle != STAGE / "bundle-v1":
        raise RuntimeError("COMPLETE17_STAGE_PATH_MISMATCH")
    ...
    command = [sys.executable, "-I", "-B", str(bundle / "complete17_cycle.py"),
    ...
    summary = {"status": "PASS" if child.returncode == 0 and report.get("status") == "PASS" and unchanged and not retired_present else "FAIL",
               "scope": "PYTHON310_FINAL17_COMBINED_SYNTHETIC_CYCLE", "overall_gate_b": "NOT_EVALUATED",
               ...
               "writers": report.get("writers", {}).get("status"),
               ...
               "error": report.get("error"), "completed_at_utc": datetime.now(timezone.utc).isoformat()}
