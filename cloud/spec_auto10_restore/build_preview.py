...
def build():
    ...
    shell_delta = spec_publication.card_shell.permitted_delta(before, after, "UA-0001")
    assert spec_publication.inject(after, "UA-0001", facts) == after
    checked = spec_publication.validate_page(after, "UA-0001", facts, previous=before)
...
<p>Проверка изменения на копии реальной страницы UA-0001. Заголовок раздела — «Додаткова специфікація». Удалены повторные VIN; основной VIN, остальное описание, цены, этапы, фотографии и видео сохранены.</p>
...
<div class="facts"><span>1 отдельный раздел</span><span>43 справочных параметра</span><span>1 видимый VIN</span><span>0 рекламных ссылок в разделе</span></div>
...
<p>Раздел в копии раскрыт для просмотра. Сценарий в CRM после исправления: сохранить данные автомобиля → автоматический сбор → обычная публикация → проверенная ссылка. Старые карточки обрабатываются без отдельных нажатий.</p>
    ...
    report = {"task": "UA-ART-SPEC-AUTO-10-RESTORE-001", "preview_type": "captured_page_with_historical_profile",
              ...
              "card": "UA-0001", "spec_rows": checked["rows"], "visible_vin_count": checked["visible_vin_count"],
              "permitted_vin_removals": shell_delta["permitted_removals"],
