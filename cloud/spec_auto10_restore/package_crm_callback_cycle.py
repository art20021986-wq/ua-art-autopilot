#!/usr/bin/env python3
"""Private deterministic CRM callback rehearsal archive; never an installer."""
import argparse
import hashlib
import json
...
PINS = {
    ...
    "repair_crm_edit_session.py": "97e1f170301d26403e115962392587f141be40a97b94acae412e3caeec8611f1",
}
...
def build(candidate, output):
    base, candidate = Path(__file__).absolute().parent, candidate.absolute()
    ...
    for name in tuple(PINS) + ("crm_callback_cycle.py", "crm_callback_server_gate.py"):
        data = read(base / name)
        if name in PINS and hashlib.sha256(data).hexdigest() != PINS[name]:
            raise RuntimeError("HELPER_SHA_MISMATCH:" + name)
        compile(data, name, "exec")
        files[name] = data
    package = {"scope": "CRM_CALLBACK_SYNTHETIC_SERVER_REHEARSAL_ONLY", "production_changed": False,
               "gate_b": "NOT_EVALUATED", "candidate_manifest_sha256": MANIFEST_SHA,
