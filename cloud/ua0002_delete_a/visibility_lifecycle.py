...
def retire_lists(binding, code, operation_id):
    ...
    for path, (before, source) in proposed.items():
        if path.name == "katalog.html":
            ...
            mode = getattr(binding, "retirement_home_modes", {}).get(str(path))
            if mode == "LEGACY_TILES":
                if "stage-card" in source or "outline-cta" in source:
                    raise VisibilityError("LEGACY_HOME_MODE_DRIFT")
                result = source
            elif mode == "MODERN_COUNTERS":
                result = counters.patch_home(source, counts)
            else:
                raise VisibilityError("HOME_MODE_NOT_BOUND")
        else:
