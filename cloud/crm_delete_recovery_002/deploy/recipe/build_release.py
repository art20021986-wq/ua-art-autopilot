...
INSTALLER = ('package_install.py', 'lifecycle_controller.py', 'lifecycle_worker.py',
...
def validate_writer_receipt(raw, sources, writers, payloads):
    ...
    if value.get('recipe_sha256') != {
        name: sha(writer_recipe_bytes(name)) for name in WRITER_RECIPE
    }:
...
def writer_recipe_bytes(name):
    """Read QA evidence as hash-only data in the admitted build closure.

    The two QA modules are never imported or executed by this builder. A
    materialized deployment stores their exact bytes as .py.txt to retain
    reproducible test provenance without declaring them runtime dependencies.
    Executed recipes always retain their .py identity and admission checks.
    """
    path = HERE / 'writer_patch' / name
    if name in ('test_writer_guard.py', 'test_publication_worker.py') and not path.exists():
...
def build(sources, output):
    ...
    receipt = {'status': 'CANDIDATE_BUILT', 'production_ready': False, 'installed': False,
               ...
               'production_blockers': ['fresh exact release-bound production authority required',
