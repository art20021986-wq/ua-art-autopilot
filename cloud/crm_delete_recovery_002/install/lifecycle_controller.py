...
TASK = 'UA-ART-CRM-DELETE-RECOVERY-002'
...
def installation_policy_sha256(plan):
    ...
    """
    static = {key: plan[key] for key in ('version', 'task_id', 'package_sources',
    ...
    static['authority_policy'] = {key: plan['authority'][key] for key in
        ('repository_root', 'control_plane_sha256', 'critical_workflow_sha256')}
...
def verify_installation_policy(plan, request=None, approval=None):
    ...
    if approval is not None:
        require(approval.get('installation_policy_sha256') == digest, 'APPROVAL_INSTALLATION_POLICY_BINDING')
    return digest


def verify_open_transaction(*, root, authority, cp, claim, request, request_sha,
                            backup_rel, backup_path, transaction_rel):
    ...
    require(set(value) == keys and value.get('schema_version') == cp.PRODUCTION_TRANSACTION_SCHEMA and
            value.get('status') == 'OPEN' and value.get('request_path') == authority['request_path'] and
            ...
            claim.get('production_transaction_status') == 'OPEN', 'PERSISTED_TRANSACTION_NOT_EXACT_OPEN')
    now = datetime.now(timezone.utc)
    prepared, opened, expiry = (cp.parse_utc(str(value[key])) for key in ('prepared_at', 'opened_at', 'expires_at'))
    require(prepared <= opened <= now < expiry, 'OPEN_TRANSACTION_TIME_BINDING')
    ...
    return {'transaction_path': transaction_rel, 'transaction_status': 'OPEN',
...
class RepositoryAdmission:
    ...
    def check(self):
        ...
        cp_path = root / 'automation/control_plane.py'
        read_bound(cp_path, authority['control_plane_sha256'])
        read_bound(root / '.github/workflows/uaart_critical.yml', authority['critical_workflow_sha256'])
        ...
        require(request.get('task_id') == TASK and request.get('production_required') is True,
        ...
        require(trusted.get('controller_sha256') == self.plan['package_sources']['lifecycle_controller.py'],
        ...
        require(approved.get('task_id') == TASK and approved.get('approval_status') == 'APPROVED' and
                approved.get('production_installation_authorized') is True and
                approved.get('operation') == 'install' and
                approved.get('package_manifest_sha256') == self.plan['package_manifest_sha256'] and
                isinstance(approved.get('owner_command'), str) and bool(approved['owner_command'].strip()),
...
def load_plan(path, expected):
    ...
    require(plan.get('version') == 1 and plan.get('task_id') == TASK, 'LIFECYCLE_PLAN_CONTRACT')
