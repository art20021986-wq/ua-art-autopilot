...
def build_candidate(source, *, expected_sha256=EXPECTED_SOURCE_SHA256):
    ...
    signatures = {
        'apply_value': ['card_id', 'field', 'raw', 'actor_id'],
        'auto_catch': ['msg', 'card', 'actor_id', 'context'],
        'catch_message': ['update', 'context'],
    }
    for name, arguments in signatures.items():
        node = function(source, name)
        if [arg.arg for arg in node.args.args] != arguments:
            raise Refused('HANDLER_SIGNATURE_DRIFT:' + name)
    original = source
    ...
    before_routing = catch.body[:catch.body.index(no_wait)]
    assignments = {target.id: node for node in before_routing if isinstance(node, ast.Assign)
                   for target in node.targets if isinstance(target, ast.Name)}
    for anchor in ('input_text', 'thinking', 'wait', 'msg', 'user_id'):
        if anchor not in assignments:
            raise Refused('CATCH_INPUT_ANCHOR:' + anchor)
    wait_value = assignments['wait'].value
    if (not isinstance(wait_value, ast.Call)
            or ast.unparse(wait_value.func) != 'context.user_data.get'
            or not wait_value.args
            or not isinstance(wait_value.args[0], ast.Constant)
            or wait_value.args[0].value != 'car_wait'):
        raise Refused('CATCH_INPUT_ANCHOR:car_wait')
