...
class V2Tests(unittest.TestCase):
    def setup_case(self, folder):
        ...
        install = {"source":"AUTHENTICATED_PYTHONANYWHERE_FILE_EDITOR","receipt":{"status":"PASS","safe_to_stop":True,"crm_resume":{"enabled":True,"state":"Running"},"installer":{"status":"PASS","task_id":fr.TASK,"target":"UA-0022","integrity_pass":True,"post_check_pass":True,"other_rows_unchanged":True,"protected_pages_unchanged":True,"target_media_unchanged":True,"target_original_media_unchanged":True,"ua_ge_prices_unchanged":True,"publication_pass":True,"source_after_sha256":{k:"c"*64 for k in fr.RUNTIME_KEYS}}}}
        health = {"schema_version":"UAART-UA0022-FRESH-HEALTH-1","main":fr.MAIN,"production_write_performed":False,"started_at":"2026-09-20T11:09:00Z","finished_at":"2026-09-20T11:10:00Z","status":"healthy","exit_code":0,"routes":{k:{"http_status":200,"final_url":"https://www.uaart.com.ua/video/index.html","sha256":"d"*64} for k in fr.HTTP_KEYS},"public_bytes_changed_after_install_observation":["ua0022_card","ua0022_diagnostic"]}
        ip = pathlib.Path(folder)/"install.json"; ip.write_bytes(b(install))
        hp = pathlib.Path(folder)/"health.json"; hp.write_bytes(b(health))
        manifest = {"main":fr.MAIN,"repo_inputs":repo_manifest,"must_not_exist":sorted(fr.ADDITIONS),"external":{"install":{"sha256":hashlib.sha256(ip.read_bytes()).hexdigest()},"health":{"sha256":hashlib.sha256(hp.read_bytes()).hexdigest()}}}
    ...
    def test_exact_success_and_scoped_receipt(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertNotIn("unexpected_changes",receipt)
