#!/usr/bin/env python3
"""Guarded CRITICAL production controller for UA110."""
...
CONTRACT = "UA-VIN-SPEC-3SRC-V1.0"
CRITICAL_CONTRACT = "UA-ART-CRITICAL-ADAPTER-V1.0"
MANIFEST_SHA256 = "ab5e94c29603656ff9e4f03d0766d0ec1f0a2add1fdf3f7f0e94629d5c474033"
BASE = "https://www.pythonanywhere.com/api/v0/user/Carix/"
REMOTE = "/home/Carix/autopilot_inbox/cloud/task_068_ferry_vin"
INSTALL_RECEIPT = REMOTE + "/task110_install_receipt.json"
VERIFY_RECEIPT = REMOTE + "/task110_verify_receipt.json"
ROLLBACK_RECEIPT = REMOTE + "/task110_rollback_receipt.json"
...
EVIDENCE_REL = "cloud/task_110_vin_spec_3src/evidence.json"
...
UPLOAD_FILES = (
    "remote_installer.py", "source_policy.py", "vin_spec_service.py", "integration_patcher.py",
)
...
def atomic_json(path: pathlib.Path, value: dict[str, Any]) -> None:
    ...
    handle = tempfile.NamedTemporaryFile(
        ...
        prefix="." + path.name + ".", suffix=".task110.tmp", delete=False,
    )
...
def rollback_drill() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="ua110-rollback-") as folder:
        path = pathlib.Path(folder) / "probe"
        original = b"UA110-ORIGINAL\n"
        path.write_bytes(original)
        before = sha_file(path)
        backup = path.read_bytes()
        path.write_bytes(b"UA110-MUTATED\n")
...
class API:
    ...
    def request(self, method: str, url: str, data: bytes | None = None,
        ...
        request_headers = {
            ...
            "User-Agent": "ua-art-task110-vin-spec/1",
        }
    ...
    def upload(self, path: str, value: bytes) -> None:
        boundary = "----uaart-task110-" + uuid.uuid4().hex
...
def fetch_public(uid: str) -> str:
    url = urllib.parse.urljoin(PUBLIC_BASE, uid + ".html") + "?task110=" + str(int(time.time() * 1000))
    request = urllib.request.Request(url, headers={"User-Agent": "ua-art-task110-live/1", "Cache-Control": "no-cache"})
    try:
...
def public_verify(expected: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for uid, meta in sorted(expected.items()):
        ...
        verified[uid] = shown
    if len(verified) < 16 or nonempty < 1:
        raise ControllerError("PUBLIC_COVERAGE")
    return {"status": "PASS", "cards": verified, "nonempty_cards": nonempty}
...
def run() -> dict[str, Any]:
    ...
    try:
        ...
        install = api.run_remote(INSTALL_COMMAND, "task110 install/backfill", INSTALL_RECEIPT, 3300)
        ...
        verify = api.run_remote(VERIFY_COMMAND, "task110 verify", VERIFY_RECEIPT, 900)
        ...
        if install and install.get("status") == "PASS" and install.get("backup"):
            try:
                rollback = api.run_remote(ROLLBACK_COMMAND, "task110 rollback", ROLLBACK_RECEIPT, 900)
                api.restart_bot()
