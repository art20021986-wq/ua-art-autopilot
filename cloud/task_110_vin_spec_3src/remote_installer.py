#!/usr/bin/env python3
"""Fail-closed production installer for UA110 VIN specification enrichment.
...
SOURCE_TARGETS = (TARGET_SOURCE_POLICY, TARGET_SERVICE, TARGET_SPEC, TARGET_CRM)
BACKUP_PARENT = REMOTE + "/task110_backups"
LATEST_BACKUP = REMOTE + "/task110_latest_backup.txt"
...
LOCK_PATH = ROOT + "/.task110_vin_spec.lock"
...
def atomic_write(path: str, value: bytes, mode: int = 0o600) -> None:
    ...
    handle = tempfile.NamedTemporaryFile(
        ...
        suffix=".task110.tmp", delete=False,
    )
...
def _fresh_modules():
    for name in ("vin_spec_service", "ua_additional_spec", "publikaciya"):
...
def verify_local_public(cards: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(checked) < 16 or pages < 32 or nonempty < 1:
        raise InstallError("PUBLIC_COVERAGE:%d:%d:%d" % (len(checked), pages, nonempty))
    return {"status": "PASS", "cards": checked, "card_count": len(checked), "page_count": pages, "nonempty_cards": nonempty}
    ...
    try:
        ...
        with service.connect_spec(True) as conn:
            ...
            job_rows = [dict(row) for row in conn.execute(
                "SELECT car_uid,status,facts_count,site_sync_status FROM vin_spec_jobs ORDER BY car_uid"
            )]
...
def selftest() -> None:
    assert source_policy.SOURCE_DOMAINS == (
        "vpic.nhtsa.dot.gov", "auto-data.net", "carwiki.co.kr",
    )
    ...
    print("UA110_REMOTE_INSTALLER_SELFTEST_PASS")
