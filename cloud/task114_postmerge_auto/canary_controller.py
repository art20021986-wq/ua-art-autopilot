...
ROOT = HERE.parents[1]
TASK_ID = "TASK114-POSTMERGE-AUTO-CANARY"
EVIDENCE_REL = "cloud/task114_postmerge_auto/evidence.json"
RECEIPT_REL = "state/receipts/TASK114-POSTMERGE-AUTO-CANARY.json"
...
def rollback_drill() -> dict[str, str]:
    with tempfile.TemporaryDirectory(prefix="uaart-task114-") as folder:
        path = pathlib.Path(folder) / "probe"
        original = b"TASK114-ORIGINAL\n"
        path.write_bytes(original)
        before = sha_file(path)
        backup = path.read_bytes()
        path.write_bytes(b"TASK114-MUTATED\n")
