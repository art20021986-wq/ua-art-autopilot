#!/usr/bin/env python3
"""Upload, execute, restart, verify and auto-rollback TASK 086."""
...
# task086 filenames keep this package isolated without creating a second writer.
REMOTE = "/home/Carix/autopilot_inbox/cloud/task_083_catalog_dedup"
FILES = {
    REMOTE + "/task086_remote_installer.py": HERE / "remote_installer.py",
    REMOTE + "/task086_stage_counter_guard.py": HERE / "stage_counter_guard.py",
}
RECEIPTS = {
    mode: REMOTE + "/task086_%s_receipt.json" % mode
    for mode in ("shadow", "apply", "postcheck", "rollback")
}
COMMANDS = {
    mode: "cd %s && python3.10 task086_remote_installer.py %s" % (REMOTE, mode)
    for mode in RECEIPTS
}
DIR_MARKER = REMOTE + "/task086_directory.ready"
...
PRESERVED_FIELDS = (
    ...
    "eta_manual", "published",
)
EVIDENCE = HERE / "evidence/production.json"
REPORT = HERE / "TASK_086_REPORT.md"
...
class API:
    ...
    def request(self, method, url, data=None, headers=None, allowed=(200,), timeout=90):
        request_headers = {
            ...
            "User-Agent": "ua-art-task086-controller/1",
        }
    ...
    def upload(self, path: str, data: bytes) -> None:
        boundary = "----uaart-task086-" + uuid.uuid4().hex
    ...
    def run_remote(self, mode: str, timeout: int = 360) -> dict[str, Any]:
        ...
        for run_attempt in (1, 2):
            ...
            trigger = self.create_trigger(
                COMMANDS[mode], "task086 %s attempt %d" % (mode, run_attempt)
            )
            try:
...
def validate_shadow(value: dict) -> None:
    ...
    if target.get("auto_number") != TARGET_CODE or target.get("status") not in ALLOWED_PRE_STATUSES:
    ...
    if projected.get("status") != EXPECTED_STATUS:
        raise ControllerError("SHADOW_PROJECTED_TARGET")
    if (value.get("canary") or {}).get("runs") != 10 or not (
        value.get("canary") or {}
    ).get("stable"):
        raise ControllerError("SHADOW_CANARY")
    preview = value.get("visual_preview") or {}
    if preview.get("status") != "PASS" or preview.get("viewports") != [390, 1440]:
        raise ControllerError("SHADOW_VISUAL_PREVIEW")
...
def validate_apply(value: dict) -> None:
    ...
    for field in PRESERVED_FIELDS:
        if field in after and after.get(field) != before.get(field):
            raise ControllerError("APPLY_PRESERVED_FIELD:" + field)
    for field in LATER_FIELDS:
        if after.get(field) not in (None, ""):
            raise ControllerError("APPLY_LATER_FIELD:" + field)
...
def validate_postcheck(value: dict) -> None:
    ...
    if target.get("status") != EXPECTED_STATUS:
        raise ControllerError("POSTCHECK_STATUS")
    for field in LATER_FIELDS:
        if target.get(field) not in (None, ""):
            raise ControllerError("POSTCHECK_LATER_FIELD:" + field)
    if database.get("legacy_triggers"):
        raise ControllerError("POSTCHECK_LEGACY_TRIGGER")
    if value.get("markers") != {
        "db.py": 1, "cars_ui.py": 1, "stranica.py": 1,
        "cars_schema.py": 1, "publikaciya.py": 1,
    }:
...
def fetch_public(path: str, nonce: str) -> str:
    ...
    request = urllib.request.Request(
        "https://www.uaart.com.ua" + path + separator + "task086=" + nonce,
        headers={"User-Agent": "ua-art-task086-public/2", "Cache-Control": "no-cache"},
    )
...
def public_round(label: str, expected_payload: dict) -> dict:
    ...
    for surface in ("video",):
        ...
        expected_container = str(expected_payload.get("sea_container") or "").strip()
        expected_eta = str(expected_payload.get("eta_manual") or "").strip()
        ...
        detail_checks = {
            ...
            "own_cover": bool(re.search(r"foto/UA-0011/m/001\.jpg", detail, re.I)),
            "vin": "4289" in detail,
            "container_preserved": not expected_container or expected_container in detail,
            "eta_preserved": not expected_eta or any(
                value and value in detail for value in eta_variants
            ),
...
def report_text(result: dict) -> str:
    ...
    public = ((result.get("public_delayed") or result.get("public_immediate") or {}).get(
        "video"
    ) or {})
    return "\n".join([
        ...
        "- Public category: `%s`" % public.get("catalog_stage", "—"),
        "- Container preserved: `%s`" % (target.get("sea_container") or "—"),
        "- ETA/days preserved: `%s` / `%s`" % (
            target.get("eta_manual") or "—", target.get("days_to_kyiv") or "—"
        ),
        "- Visible counters: `%s`" % json.dumps(
            public.get("visible_counts") or {}, ensure_ascii=False, sort_keys=True
        ),
        "- Permanent stage/counter transaction: %s" % (
...
def main() -> int:
    ...
    try:
        ...
        expected_payload = apply_value.get("before") or {}
        result["public_immediate"] = public_round("immediate", expected_payload)
        time.sleep(35)
        delayed = api.run_remote("postcheck")
        result["postcheck_delayed"] = delayed
        validate_postcheck(delayed)
        result["public_delayed"] = public_round("delayed", expected_payload)
        result["status"] = "PASS"
    except Exception as exc:
