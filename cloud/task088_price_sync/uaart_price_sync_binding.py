...
class Provider:
    ...
    def _validate_delegation(self):
        d = self.delegation
        _same(d.get("contract"), CONTRACT, "EXACT_DELEGATION_CONTRACT_REQUIRED")
        ...
        _same(d.get("operation"), "UPDATE_EXISTING_CARD_AND_CATALOG_PRICES", "PRICE_ONLY_DELEGATION_REQUIRED")
        ...
        if (type(identities) is not list or len(identities) != 18
                or any(type(pair) is not list or len(pair) != 2 or type(pair[0]) is not int or pair[0] <= 0
                       or re.fullmatch(r"UA-[0-9]{4}", str(pair[1])) is None for pair in identities)
                or len({pair[0] for pair in identities}) != 18
        ...
        if type(self.code_pins) is not dict or not REQUIRED_CODE <= set(self.code_pins):
    ...
    def resolve_surfaces(self, code):
        ...
        if type(raw) is not list or not raw or {item.get("kind") for item in raw} != {"CARD", "CATALOG"}:
            raise BindingError("EXACT_CARD_AND_CATALOG_SURFACES_REQUIRED")
        result = []
        for item in raw:
            if set(item) != {"kind", "path", "url"}:
            ...
            wanted = code + ".html" if item["kind"] == "CARD" else "katalog.html"
            if path.name != wanted or parsed.path.rsplit("/", 1)[-1] != wanted:
                raise BindingError("PUBLIC_SURFACE_IDENTITY_MISMATCH")
            result.append(runtime.Surface(item["kind"], path, item["url"]))
    ...
    def authorize(self, event, surfaces):
        ...
        with sqlite3.connect(self.db_path.as_uri() + "?mode=ro", uri=True, timeout=1) as conn:
            actual = outbox.get(conn, key)
            if actual is None or actual != event or actual["state"] not in ("CLAIMED", "STOPPED"):
        ...
        record = {"authority_kind": "CANONICAL_DEPLOYMENT_DELEGATED_LOCAL_PRICE_EVENT",
            ...
            "event_sha256": sha(encoded(event)), "revision": event["revision"],
    ...
    def binding(self):
        ...
        return runtime.Binding(db_path=self.db_path, publication_lock=self.lock_path,
            ...
            owner_private_chat_verified=True, car_identities=self.identities, clock=self.clock)
