...
CONTRACT = "UA-ART-GE-PRICE-STAGE3-INSTALL-1"
...
DEPENDENCIES = frozenset({"db.py", "cars_schema.py", "start_safe.py", "ua_stage_catalog_sync.py",
                         "master_card.py", "publikaciya.py",
                         "catalog_design_golden.html"})
EVIDENCE_NAMES = ("request", "claim", "transaction", "gate_b", "stage2", "quota", "writers", "manifest", "owner_approval")
...
def build_candidates(source_files, html_files, rows, modules):
    ...
    expected |= {"video/katalog.html", "site/katalog.html"}
    ...
    for name, original in html_files.items():
        ...
        code = Path(name).stem
        value, _ = migrate_catalog(text, list(by_code.values())) if code == "katalog" else migrate_card(text, by_code[code])
...
def _validate(plan, files, evidence, now, root, *, testing):
    ...
    if set(evidence) != set(EVIDENCE_NAMES) or set(plan.get("evidence_sha256", {})) != set(EVIDENCE_NAMES):
    ...
    request, claim, transaction, gate, stage2, quota, writers, approved_manifest, owner = (records[name] for name in EVIDENCE_NAMES)
    ...
    if (claim.get("task_execution_status") != "RUNNING" or claim.get("production_transaction_id") != plan["transaction_id"]
            ...
            or transaction.get("status") != "OPEN" or transaction.get("transaction_id") != plan["transaction_id"]
            or transaction.get("run_id") != identity.get("run_id") or not str(identity.get("run_id", "")).isdigit()
            or _instant(transaction.get("expires_at")) <= now):
    ...
    if (owner.get("schema_version") != "UA-ART-PRODUCTION-AUTHORIZATION-1"
            ...
            or owner.get("request_subject_sha256") != subject_hash or _instant(owner.get("expires_at")) <= now
...
def install(plan, files, evidence, *, expected_plan_sha256, schema_installer, test_root=None, now=None):
    ...
    if len(codes) != plan.get("published_count") or (not testing and len(codes) != 18):
    ...
    try:
        ...
        required = 3 * (db_bytes + sum(len(value or b"") + len(files[name]) for name, value in originals.items())
                        + sum(len(value) for value in dependencies.values())) + 1024 * 1024
        quota = records["quota"]
        ...
        permitted_tables = {"uaart_price_publication_outbox_v1", "uaart_price_publication_recovery_v1",
                            "uaart_price_sync_notices_v1"}
        if any(row[2] not in permitted_tables or row[0] not in ("table", "index", "trigger")
               for row in after_schema if row not in before_schema):
