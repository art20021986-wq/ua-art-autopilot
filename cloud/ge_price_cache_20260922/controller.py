...
REMOTE_SCRIPT="/home/Carix/uploads/ge_price_cache_20260922.py"
REMOTE_RECEIPT="/home/Carix/uploads/ge_price_cache_20260922.json"
...
class API:
    ...
    def run(self,mode,run_id,backup_sha=""):
        ...
        cmd="cd /home/Carix/uploads && python3.10 ge_price_cache_20260922.py %s %s%s"%(mode,run_id,arg)
        i=self.schedule(cmd)
        try:
    ...
    for attempt in range(8):
        try:
            ...
            with urllib.request.urlopen('https://www.uaart.com.ua/video/ua-site-languages.js',timeout=30) as r:
                js=r.read(MAX+1)
            if sha(js)!='f3cfcd38b3874e10905c20162b992ffd04afe0294999db9bfa4624cddfb1d645':raise E('LIVE_JS_HASH')
            pages={}
            for path in ('katalog.html','UA-0010.html','UA-0007.html'):
                with urllib.request.urlopen('https://www.uaart.com.ua/video/'+path,timeout=30) as r:
                    body=r.read(MAX+1)
                    if r.status!=200 or b'/video/ua-site-languages.js?v=ge-prices-20260922-v1' not in body: raise E('LIVE_PAGE')
                    pages[path]=sha(body)
...
def main():
    ...
    try:
        ...
        final={'contract_id':CONTRACT,'task_id':TASK_ID,'status':'FINISHED','task_class':'CRITICAL','target_environment':'production','tests':'PASS','backup':'PASS','production':'PASS','live_verify':'PASS','rollback':'PASS','unexpected_changes':0,'protected_files_unchanged':True,'crm_unchanged':True,'manifest_sha256':env['UAART_MANIFEST_SHA256'],'request_sha256':os.environ['UAART_REQUEST_SHA256'],'run_id':env['UAART_RUN_ID'],'transaction_id':os.environ['UAART_TRANSACTION_ID'],'rollback_ready':True,'production_required':True,'production_touched':True,'live':live}
