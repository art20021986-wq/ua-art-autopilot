...
CATALOG_PRICE_COPY = ("%d в подборке · цены под ключ в Киеве", "%d в подборке · цены для Украины и Грузии")
...
def patch_stranica(source_bytes):
    ...
    old, new = CATALOG_PRICE_COPY
    if _node_text(source, catalog).count(old) != 1 or candidate.count(old) != 1:
        raise ValueError("STRANICA_CATALOG_PRICE_COPY_MISMATCH")
    ...
    return encoded, {
        ...
        "price_statements_replaced": 2, "old_caption_removed": 1,
        "catalog_price_copy_replacements": 1,
