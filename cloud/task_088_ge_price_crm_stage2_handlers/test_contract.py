...
class ContractTests(unittest.TestCase):
    ...
    def test_redacted_handler_never_exports_setting_or_arbitrary_literals(self):
        source = '''async def catch_message(update, context):
    token = "short-secret"
    x = "unlabelled-sensitive-value"
    wait = context.user_data.get("car_wait")
    if wait and wait.get("field") == "price_georgia":
        return apply_value(wait["card_id"], wait["field"], update.text, 0)
'''
        result = c.inspect_source(source.encode(), c.SOURCE_PATHS[0])
        selected = result["selected_functions"]["catch_message"][0]
        self.assertFalse(selected["source_exported"])
        self.assertTrue(selected["analysis_only_not_executable_source"])
        text = selected["redacted_analysis_source"]
        self.assertNotIn("short-secret", text)
        self.assertNotIn("unlabelled-sensitive-value", text)
        self.assertIn("price_georgia", text)
        self.assertIn("car_wait", text)
        self.assertIn("apply_value", text)
        ast.parse(text)
