...
TASK_ID = "CRM-SITE-CONSISTENCY-PROBE-20260926"
...
def serving(token: str) -> dict:
    ...
    out["sync_modules"] = {}
    for name in ("ua_crm_public_sync.py", "ua_public_freshness.py", "ua_price_html_v2.py"):
        source = api_file(token, "/home/Carix/" + name)
        if source is not None:
            out["sync_modules"][name] = {
                "sha256": sha(source.encode()), "bytes": len(source.encode()),
                "lines": probe.redact_lines(source, SOURCE_PATTERNS +
                    (r"verify_|snapshot|revision|retry|published|SELECT|def ",), 100)}
    return out
...
def execute(environment: Mapping[str, str]) -> dict[str, Any]:
    ...
    receipt = {
        ...
        "rollback_ready": True,
        "rollback_reason": "GET/HEAD only; no production mutation to roll back",
        ...
        "acceptance_scope": "Ukraine price presence only; not full CRM or browser acceptance",
