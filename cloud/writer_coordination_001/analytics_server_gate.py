#!/usr/bin/env python3
"""Test the reviewed analytics writer function on PA against synthetic pages.

The launcher reads its frozen bundle plus one pinned production source, then creates a unique run directory.
...
STAGE = Path("/home/Carix/analytics_writer_gate_20260909")
SOURCE = Path("/home/Carix/analitika_wsgi.py")
SOURCE_SHA256 = "a73be46099596322dcd607ecadd56140d45483a5ad38f1c1a0a0e395cfc8bc94"
BUNDLE_FILES = {"patch_analytics_writer.py", "tests/test_analytics_writer.py", "analytics_server_gate.py"}
...
def verify_bundle(bundle):
    ...
    if manifest.get("schema") != "UA-ART-ANALYTICS-WRITER-TEST-BUNDLE-1" or set(manifest.get("files", {})) != BUNDLE_FILES:
    ...
    if set(manifest.get("test_counts", {})) != {"test_analytics_writer.py"}:
...
def run_tests(bundle, output):
    ...
    captured = output / "inputs" / "analitika_wsgi.py"
    if os.environ.get("UAART_ANALITIKA_SOURCE") != str(captured) or file_snapshot(captured)["sha256"] != SOURCE_SHA256:
        raise RuntimeError("ANALYTICS_CAPTURE_BINDING")
    suite.addTests(unittest.TestLoader().discover(str(bundle / "tests"), pattern="test_analytics_writer.py"))
    ...
    report = {
        "schema": "UA-ART-ANALYTICS-WRITER-SERVER-TEST-1",
        ...
        "scope": "ISOLATED_ACTUAL_ANALYTICS_FUNCTION_WITH_SYNTHETIC_HTML",
        ...
        "source_sha256": manifest["files"], "captured_source_sha256": SOURCE_SHA256, "production_changed": False,
...
def capture_source():
    ...
    try:
        ...
        if not stat.S_ISREG(initial.st_mode) or initial.st_nlink != 1 or initial.st_size > 2 * 1024 * 1024:
            raise RuntimeError("ANALYTICS_SOURCE_TYPE")
        with os.fdopen(os.dup(descriptor), "rb") as stream:
            data = stream.read()
        current, named = os.fstat(descriptor), SOURCE.lstat()
        identity = lambda value: (value.st_dev, value.st_ino, value.st_size, value.st_mtime_ns)
        if identity(initial) != identity(current) or identity(current) != identity(named) or sha(data) != SOURCE_SHA256:
            raise RuntimeError("ANALYTICS_SOURCE_DRIFT")
        return data, {"sha256": sha(data), "identity": identity(current)}
    finally:
        os.close(descriptor)
...
def main():
    ...
    source_data, source_before = capture_source()
    output.mkdir(mode=0o700)
    inputs = output / "inputs"
    inputs.mkdir(mode=0o700)
    captured = inputs / "analitika_wsgi.py"
    with captured.open("xb") as stream:
        stream.write(source_data)
        stream.flush()
        os.fsync(stream.fileno())
    child = subprocess.run([sys.executable, "-I", "-B", str(bundle / "analytics_server_gate.py"), "--child", str(output)],
        cwd=bundle, env={"PATH": "/usr/bin:/bin", "PYTHONDONTWRITEBYTECODE": "1", "UAART_ANALITIKA_SOURCE": str(captured)},
    ...
    _after_data, source_after = capture_source()
    source_unchanged = source_before == source_after
    unchanged = all(file_snapshot(bundle / name) == previous for name, previous in before.items()) and source_unchanged
    summary = {
        "schema": "UA-ART-ANALYTICS-WRITER-SERVER-SUMMARY-1", "python_version": sys.version,
        ...
        "bundle_inputs_unchanged": unchanged, "production_source_unchanged": source_unchanged, "production_source_sha256": SOURCE_SHA256, "production_changed": False,
        "real_task_api_called": False, "production_lock_files_opened": False,
