...
class Tests(unittest.TestCase):
    ...
    def test_old_installed_receipt_not_current_after_rollback(self):
        ...
        self.assertEqual(self.new_worker().read_terminal(), result)
        self.assertEqual(result['status'], 'CODE_ROLLED_BACK_READBACK')
    def test_shared_filesystem_percent_does_not_fake_account_quota(self):
        class Shared:
            f_bavail=1_000_000
            f_frsize=4096
            f_blocks=10_000_000_000
        with patch.object(h.os,'statvfs',lambda p: Shared()):
            self.assertEqual(self.worker.install()['status'],'CODE_INSTALLED_READBACK')
    def test_filesystem_absolute_capacity_still_blocks(self):
        class Full:
            f_bavail=0
            f_frsize=4096
        with patch.object(h.os,'statvfs',lambda p: Full()):
            with self.assertRaisesRegex(h.HandoffError,'FILESYSTEM_FREE_BYTES_INSUFFICIENT'):
                self.worker.install()
        self.assert_before()
    def test_account_quota_80_heavy_gate(self):
        original=self.authority.__call__
        self.worker.verifier=lambda p,c,phase: {**original(p,c,phase),
            'storage_quota': {'source':'AUTHENTICATED_PYTHONANYWHERE_ACCOUNT_QUOTA',
                             'used_bytes':28_000_000_000,'quota_bytes':35_000_000_000,
                             'observed_at':time.time(),'receipt_sha256':'d'*64}}
        with self.assertRaisesRegex(h.HandoffError,'ACCOUNT_QUOTA_HEAVY_DEPLOY_GATE_80'):
            self.worker.install()
        self.assert_before()
    def test_missing_account_quota_rejected(self):
        self.authority.custom={'storage_quota':None}
        with self.assertRaisesRegex(h.HandoffError,'AUTHENTICATED_ACCOUNT_QUOTA_REQUIRED'):
            self.worker.install()
        self.assert_before()
    def test_account_quota_90_emergency_gate(self):
        self.authority.custom={'storage_quota': {'source':'AUTHENTICATED_PYTHONANYWHERE_ACCOUNT_QUOTA',
            'used_bytes':32_000_000_000,'quota_bytes':35_000_000_000,
            'observed_at':time.time(),'receipt_sha256':'d'*64}}
        with self.assertRaisesRegex(h.HandoffError,'ACCOUNT_QUOTA_EMERGENCY_STOP_90'):
            self.worker.install()
        self.assert_before()
    def test_account_quota_70_warning_keeps_install_allowed(self):
        self.authority.custom={'storage_quota': {'source':'AUTHENTICATED_PYTHONANYWHERE_ACCOUNT_QUOTA',
            'used_bytes':25_000_000_000,'quota_bytes':35_000_000_000,
            'observed_at':time.time(),'receipt_sha256':'d'*64}}
        self.worker.install()
        event=next(e for e in self.worker.events if e['event']=='STORAGE_PREFLIGHT')
        self.assertTrue(event['account_warning_70'])
    def test_stale_account_quota_rejected(self):
        self.authority.custom={'storage_quota': {'source':'AUTHENTICATED_PYTHONANYWHERE_ACCOUNT_QUOTA',
            'used_bytes':1_000_000_000,'quota_bytes':35_000_000_000,
            'observed_at':time.time()-31,'receipt_sha256':'d'*64}}
        with self.assertRaisesRegex(h.HandoffError,'AUTHENTICATED_ACCOUNT_QUOTA_REQUIRED'):
            self.worker.install()
        self.assert_before()
