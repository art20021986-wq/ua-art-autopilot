...
STAGE = Path("/home/Carix/code_handoff_v2_gate_20260909")
BUNDLE_FILES = {
    "server_fence.py", "code_handoff_v2.py", "test_code_handoff_v2.py", "handoff_v2_server_gate.py",
}
...
def verify_bundle(bundle):
    ...
    if manifest.get("schema") != "UA-ART-CODE-HANDOFF-V2-TEST-BUNDLE-1" or set(manifest.get("files", {})) != BUNDLE_FILES:
    ...
    if set(manifest.get("test_counts", {})) != {"test_code_handoff_v2.py"}:
...
def run_tests(bundle, output):
    ...
    suite.addTests(unittest.TestLoader().discover(str(bundle), pattern="test_code_handoff_v2.py"))
    ...
    report = {
        "schema": "UA-ART-CODE-HANDOFF-V2-SERVER-TEST-1",
        ...
        "scope": "ISOLATED_SYNTHETIC_17_FILES_REAL_LOCKS_FAKE_AUTHORITY",
...
def main():
    ...
    child = subprocess.run([sys.executable, "-I", "-B", str(bundle / "handoff_v2_server_gate.py"), "--child", str(output)],
    ...
    unchanged = all(file_snapshot(bundle / name) == previous for name, previous in before.items())
    summary = {
        "schema": "UA-ART-CODE-HANDOFF-V2-SERVER-SUMMARY-1", "python_version": sys.version,
