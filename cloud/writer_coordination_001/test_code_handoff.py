...
class Tests(unittest.TestCase):
    def setUp(self):
        ...
        manifest = {'candidate_id':'UA-ART-SPEC-AUTO10-COMPLETE-17-V1','module_count':17,
...
'''
        ...
        def check(name,data,expected,**kwargs):
            result = subprocess.run([sys.executable,'-c',code]+[str(self.root/n) for n in sf.LOCK_NAMES],timeout=10)
            self.assertEqual(result.returncode,0); original(name,data,expected,**kwargs)
