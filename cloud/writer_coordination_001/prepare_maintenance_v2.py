...
import re
import code_handoff_v3 as handoff
...
REQUIRED_EVIDENCE = {
    ...
    'overall_gate_b_and_source10': 'Actual final17v2 execution plus permitted source10 evidence and remaining full Gate B checks',
...
def prepare(candidate, inventory, live=None):
    ...
    pins = {name:hashlib.sha256((HERE/name).read_bytes()).hexdigest() for name in
            ('code_handoff_v3.py','server_fence.py','platform_control.py','handoff_v3_server_gate.py')}
    plan = {'schema':'UA-ART-MAINTENANCE-READINESS-V2','status':'BLOCKED_BEFORE_EXACT_EXECUTION_PLAN',
            ...
            'terminal_source_pins':pins,'required_evidence':evidence,
