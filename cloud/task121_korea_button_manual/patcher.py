...
NEW_CALLBACK_PATTERN = r"^car_setstage:\d+:(?:sea_loaded|sea_transit|ua_handed)$"
...
def _status_map(source: str) -> dict[str, tuple[int, str]]:
    ...
    matches: list[dict[str, tuple[int, str]]] = []
    ...
    if len(matches) != 1:
        raise PatchError("STATUSES_SHAPE")
    return matches[0]
...
def patch_cars_ui(source: str) -> str:
    ...
    old = 'pattern=r"%s"' % OLD_CALLBACK_PATTERN
    new = 'pattern=r"%s"' % NEW_CALLBACK_PATTERN
    if source.count(new) == 1 and source.count(old) == 0:
        result = source
...
def verify(cars_ui: str, konteyner: str, schema: str) -> dict[str, Any]:
    ...
    if (
        OLD_CALLBACK_PATTERN in cars_ui
        or OLD_HIDDEN_LITERAL in cars_ui
        or OLD_HIDDEN_LITERAL in konteyner
    ):
