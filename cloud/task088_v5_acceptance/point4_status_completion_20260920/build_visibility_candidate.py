...
def main():
    ...
    parser.add_argument("--ua0022-inputs", type=Path, required=True)
    ...
    record(evidence / "COMBINED_BUILD_INTENT.json", {
        ...
        "input_scope": "Exact retained UA0022 successor source bytes composed with historical HTML, rows, and remaining source/dependency inputs. No current live observation is claimed.",
    ...
    counters_pin = "500ca67145faa38ca9f72ac6da85e2a7d1c351f8f2fcca4a2edeb34c22734c23"
    if historical["system_inventory"]["ua_site_counters.py"]["sha256"] != counters_pin:
        raise RuntimeError("HISTORICAL_COUNTERS_INVENTORY_PIN_MISMATCH")
    source_bytes["ua_site_counters.py"] = read(args.historical_inputs / "ua_site_counters.py", counters_pin)
    ...
    for name, pin in ua0022_pins.items():
        data = read(args.ua0022_inputs / name, pin)
    ...
    private_roots = [args.historical_inputs.resolve(), args.ua0022_inputs.resolve(), args.routing_inputs.resolve(), args.dependency_inputs.resolve(), output]
    ...
    for name in candidate.keys() - before.keys():
        before[name] = None
    before.update(ua0022_pins)
    before["ua_site_counters.py"] = counters_pin
    ...
    record(evidence / "COMBINED_CANDIDATE_MANIFEST.json", {
        "contract": "PR114-POINT4-UA0022-SUCCESSOR-HISTORICAL-HTML-OFFLINE-CANDIDATE-1",
    ...
    report = {
        ...
        "ua0022_input_sha256": ua0022_pins,
        "input_scope": "Retained UA0022 exact successor source inputs plus historical remaining inputs, 18 published rows, and 40 HTML. This is not a current complete server snapshot.",
        ...
        "candidate_manifest_sha256": digest(canonical(manifest)),
        "source_generation": f"Actual canonical composition of {len(SOURCES)} exact sources succeeded; all {len(compiled)} Python outputs compile. Behavioural source-generation acceptance requires independent review and already-scoped integration evidence.",
