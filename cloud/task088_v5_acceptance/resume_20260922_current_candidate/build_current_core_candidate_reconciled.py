...
def main():
    ...
    record(evidence / 'INTENT.json', {'reconciliation_adapter_sha256':reconciliation_adapter_sha256, 'reason':'Compose the final scoped point4 candidate from the newly observed current published rows, exact current core HTML, source/dependency pins and explicit module presence. Previous scoped checks are reused; no application suites or installation are run.',
    ...
    observed_html = dict(html)
    html, reconciliation = reconcile_catalogs(html, rows, raw_report)
    ...
    for name,raw in sorted(html.items()):
        ...
        proofs[name] = {'before_sha256':sha(observed_html[name]), 'after_sha256':sha(candidate[name]),
            'intermediate_before_strict_migration_sha256':sha(raw),
            ...
            'all_bytes_unchanged':observed_html[name] == candidate[name]}
    for relative,pin in code_pins.items():
        read(repo / relative, pin)
    read(reconciliation_adapter, reconciliation_adapter_sha256)
    before = {name:sha(raw) for name,raw in {**sources, **observed_html}.items()}
    ...
    record(evidence / 'CATALOG_RECONCILIATION.json', reconciliation)
    ...
    result = {'contract':'PR114-CURRENT-CORE-CANDIDATE-COMPOSITION-1', 'status':'PASS_SCOPED_CURRENT_CORE_COMPOSITION',
        ...
        'installation_authority':False, 'historical_suites_rerun':False,
        'catalog_reconciliation_sha256':sha(read(evidence / 'CATALOG_RECONCILIATION.json')),
        'reconciliation_adapter_sha256':reconciliation_adapter_sha256,
        'strict_migration_source_changed':False, 'actual_original_before_images_preserved':True,
