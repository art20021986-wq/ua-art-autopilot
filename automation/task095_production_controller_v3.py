#!/usr/bin/env python3
"""PythonAnywhere controller for TASK 095 v3."""
...
LOCAL_REMOTE = ROOT / "cloud/task_095_catalog_visual_repair/remote_repair_v3.py"
EVIDENCE = ROOT / "cloud/task_095_catalog_visual_repair/evidence_v3"
EVIDENCE.mkdir(parents=True, exist_ok=True)
REMOTE_DIR = "/home/Carix/autopilot_inbox/cloud/task_095_catalog_visual_repair"
REMOTE_SCRIPT = REMOTE_DIR + "/task095_remote_repair_v3.py"
RECEIPTS = {
    mode: REMOTE_DIR + "/%s_v3_receipt.json" % mode
...
class API:
    ...
    def request(
        ...
        all_headers = {
            ...
            "User-Agent": "ua-art-task095-v3-controller/1",
        }
    ...
    def upload(self, path: str, data: bytes) -> None:
        boundary = "----uaart-task095v3-" + uuid.uuid4().hex
    ...
    def run_remote(self, mode: str, timeout: int = 600) -> dict[str, Any]:
        receipt = RECEIPTS[mode]
        self.delete(receipt)
        command = "cd %s && python3.10 %s %s" % (
            REMOTE_DIR,
            pathlib.PurePosixPath(REMOTE_SCRIPT).name,
            mode,
        )
        trigger = self.create_trigger(command, "TASK095 v3 %s one-shot" % mode)
        try:
            ...
            while time.monotonic() < deadline:
                ...
                if raw:
                    try:
                        ...
                        raise ControllerError("RECEIPT_INVALID:" + mode) from exc
                time.sleep(5)
...
def validate(value: dict[str, Any], mode: str) -> None:
    ...
    if value.get("crm_write") is not False or value.get("media_write") is not False:
        raise ControllerError("REMOTE_SCOPE_VIOLATION")
    if mode == "install":
        ...
        if len(audits) != 2 or any((item or {}).get("status") != "PASS" for item in audits.values()):
            raise ControllerError("INSTALL_AUDITS_INVALID")
...
def main() -> int:
    ...
    try:
        ...
        value = api.run_remote(args.mode)
